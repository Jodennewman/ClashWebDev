...
exports.initCaseStudyInteractions = initCaseStudyInteractions;
// src/case-studies.ts
const animation_service_1 = require("../services/animation-service");
function initCaseStudyInteractions() {
    console.log('Initializing case study interactions');
    ...
    if (!caseButtons.length || !caseStudyBox || !closeBoxBtn) {
        console.warn('Case study elements not found:', {
            buttons: caseButtons.length,
            box: !!caseStudyBox,
            closeBtn: !!closeBoxBtn
        });
        return;
    }
    ...
    caseButtons.forEach(button => {
        ...
        const hoverTl = animation_service_1.gsap.timeline({ paused: true });
        if (buttonSVG && label) {
        ...
        button.addEventListener('click', (e) => {
            const caseNumber = e.currentTarget.getAttribute('data-case');
            if (caseNumber) {
                showCaseStudyBox(caseNumber);
            }
            // Click animation
            if (buttonSVG) {
                animation_service_1.gsap.to(buttonSVG, {
                    scale: 0.9,
                    duration: 0.1,
                    ease: "power4.out",
                    yoyo: true,
    ...
    animation_service_1.gsap.set(caseButtons, {
        opacity: 1,
        scale: 1,
        visibility: 'visible'
    });
    ...
    function showCaseStudyBox(caseNumber) {
        // Update content
        const caseData = caseStudyData[caseNumber];
        if (!caseData)
            return;
        if (!caseStudyBox)
            return;
        ...
        if (contentElement)
            contentElement.innerHTML = caseData.content;
        // Show and animate the box
        caseStudyBox.style.display = 'block';
        // Animation
        animation_service_1.gsap.fromTo(caseStudyBox, {
            opacity: 0,
            scale: 0.8,
            y: 20
        }, {
        ...
        animation_service_1.gsap.to(backdrop, { opacity: 1, duration: 0.3 });
    ...
    function closeCaseStudyBox() {
        ...
        animation_service_1.gsap.to(caseStudyBox, {
            opacity: 0,
            scale: 0.8,
            y: 20,
            duration: 0.3,
        ...
        if (backdrop) {
            animation_service_1.gsap.to(backdrop, {
                opacity: 0,
                duration: 0.3,
                onComplete: () => {
                    backdrop.remove();
