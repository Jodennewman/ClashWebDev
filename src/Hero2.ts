...
(async () => {
    ...
    function createEyeball(): void {
      if (eyeballs.length >= MAX_EYEBALLS) return; // 🛑 Prevent spawning if full
        ...
        const body = Matter.Bodies.circle(spawnX, spawnY, radius+1  , { restitution: 0.005, friction: 0.2, density: 0.2 });
    ...
    function TextSplitter (text: HTMLElement, charsYes: boolean, wordsYes: boolean, ease: string = "none", duration: any = 1, stagger: number = 0.1, position: any = "<"): { tl: gsap.core.Timeline; split: SplitText} {
 ...
 /*   .to("#heroSection2", {
    ...
    function update(): void {
        ...
        eyeballs.forEach((obj, index) => {
          ...
          const velocityMagnitude = Math.sqrt(velocityX ** 2 + velocityY ** 2) || 0.001; // Avoid division by zero

          // ✅ Normalize velocity vector (direction of movement)
          const directionX = velocityX / velocityMagnitude || 0;
          const directionY = velocityY / velocityMagnitude || 1; // Default to downward motion
